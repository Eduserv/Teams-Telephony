...
$DateRanges = @()
$HolidayDefName = 'UK Holidays 2022/23'

$region = "england-and-wales" # Accepted values "england-and-wales" / "scotland" / "northern-ireland"
 
$hols = $(ConvertFrom-Json (Invoke-WebRequest -Uri "https://www.gov.uk/bank-holidays.json").Content).$region
$today = get-date

foreach($hol in $($hols.events)){

    if($([datetime]$hol.date) -lt $today){continue} #Dont add past events

    $startdate = ([datetime]$hol.date).tostring('dd/MM/yyyy')
    $enddate = ([datetime]$hol.date).AddDays(1).tostring('dd/MM/yyyy')
    $DateRanges += New-CsOnlineDateTimeRange -Start $startdate -End $enddate 
}

# Add any additional custom values that have been specified by customer by adding lines below
# $DateRanges += New-CsOnlineDateTimeRange -Start "01/08/2022" -End "02/08/2022" #closing for renovations
# $DateRanges += New-CsOnlineDateTimeRange -Start "01/12/2023" -End "02/12/2023" #extra day off


$HolidayDef = New-CsOnlineSchedule -Name $HolidayDefName -FixedSchedule -DateTimeRanges $DateRanges
...
$EmergencyNotifiactionGroup = "CloudSolutions@eduserv.onmicrosoft.com"
...
New-CsOnlineVoiceRoute -Name "UK-Non-Geographic-Local" -Priority 0 -OnlinePstnUsages "UK-Non-Geographic-Local" -OnlinePstnGatewayList $CustomerSBCAddress -NumberPattern '^\+440?3(([2-8]\d\d|9[0-8]\d|99[0-8])\d{1,5})' -Description "Local routing for Non-Geographic, United Kingdom" | Out-Null
...
New-CsTeamsIPPhonePolicy -Identity ‘CAP’ -Description ‘Common Area Phone User Policy’ -SignInMode CommonAreaPhoneSignIn -AllowHotDesking:$false
New-CsTeamsIPPhonePolicy -Identity ‘CAP-Secure’ -Description ‘Common Area Phone User Policy without Directory Search’ -SignInMode CommonAreaPhoneSignIn -AllowHotDesking:$false -SearchOnCommonAreaPhoneMode 'Disabled'
